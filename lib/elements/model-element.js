...
export class AgentModelElement extends HTMLElement {
  ...
  #drawAgent(agent) {
    ...
    if (agent.shape.drawBackground) {
      this.#canvas.beginPath();
      agent.shape.drawBackground(this.#canvas.activeLayer.context, agent.color);
      this.#canvas.stroke();
      this.#canvas.fill();
    }

    this.#canvas.beginPath();
    agent.shape.drawPath(this.#canvas.activeLayer.context, agent.color);
    this.#canvas.stroke();
    this.#canvas.fill();
  ...
  #onClick = (event) => {
    ...
    const clickedAgents = this.agentsAt(fieldX, fieldY);

    this.dispatchEvent(new FieldEvent("fieldclick", clickedField));
    clickedField.dispatchEvent(new FieldEvent("click", clickedField));

    for (const clickedAgent of clickedAgents) {
      this.dispatchEvent(new AgentEvent("agentclick", clickedAgent));
      clickedAgent.dispatchEvent(new AgentEvent("click", clickedAgent));
    }
  };
}
